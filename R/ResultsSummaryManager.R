#' R6 Class Representing a Results Summary Manager.
#'
#' @description
#' An R6 class to represent a manager for generating summary metrics and/or matrices
#' from simulation results, as well as optionally from (regenerated) niche carrying
...
ResultsSummaryManager <- R6Class("ResultsSummaryManager",
  ...
  public = list(
    ...
    #' @description
    #' Initialization method optionally copies attributes from a multi-simulation (results) manager, sets any included attributes (sample_data, results_model, niche_k_model, human_density_model, result_attachment_functions, summary_metrics, summary_functions, parallel_cores, results_dir, results_filename_attributes), and attaches other attributes individually listed.
    #' @param simulation_manager Optional simulation manager object of a class inherited from \emph{GenericResultsManager}, from which simulation attributes can be copied.
    ...
    initialize = function(simulation_manager = NULL, ...) {
      if (!is.null(simulation_manager)) { # copy attributes
        if (!("GenericResultsManager" %in% class(simulation_manager))) {
          stop("Simulation manager must be a GenericResultsManager or inherited class object", call. = FALSE)
        }
        self$sample_data <- simulation_manager$sample_data
        self$niche_k_model <- simulation_manager$niche_k_model
        self$human_density_model <- simulation_manager$human_density_model
    ...
    generate = function() {
      ...
      # Ensure sample data, results directory & model, and summary metrics and/or matrices, and functions are present
      missing_parameters <- names(which(unlist(list(sample_data = is.null(self$sample_data),
                                                    results_dir = is.null(self$results_dir),
                                                    results_model = is.null(self$results_model),
      ...
      results_file <- file.path(self$results_dir, paste0(self$get_results_filename(1), ".RDS"))
      if (!file.exists(results_file)) {
        stop(sprintf("Could not find (first) results file %s.RDS. Ensure results_filename_attributes is set to match the filename pattern", file.path(self$results_dir, self$get_results_filename(1))), call. = FALSE)
      }
      ...
      generation_log <- foreach(i = 1:nrow(self$sample_data), .errorhandling = c("pass")) %dopar% {
        ...
        # Clone and populate the results model and copy any additional attached attributes
        results_model <- self$results_model$new_clone(results = results)
        additional_attributes <- self$results_model$get_attribute_names()[which(!(self$results_model$get_attribute_names() %in% results_model$get_attribute_names()))]
        results_model$set_attributes(self$results_model$get_attributes(additional_attributes))
        additional_attributes <- self$results_model$all$get_attribute_names()[which(!(self$results_model$all$get_attribute_names() %in% results_model$all$get_attribute_names()))]
        results_model$all$set_attributes(self$results_model$all$get_attributes(additional_attributes))

        # Generate and add optional simulation input carrying capacities and/or human densities to the results model
        sample_list <- as.list(self$sample_data[i, ])
        if (!is.null(self$niche_k_model)) {
          results_model$set_attributes(params = self$niche_k_model$generate(input_values = sample_list[self$niche_k_model$inputs]))
        }
        if (!is.null(self$human_density_model)) {
          results_model$set_attributes(params = self$human_density_model$generate(input_values = sample_list[self$human_density_model$inputs]))
        }

        # Apply result attachment functions to the results (model)
        if (!is.null(self$result_attachment_functions)) {
          self$calculate_result_attachments(results_model)
        }

        # Apply the summary functions to the results (model)
        return(self$calculate_summaries(results_model, i))
    ...
    #' @param results_model The sample results model, an object of a class inherited from \emph{GenericResultsModel}, to which the intermediate results are attached.
    calculate_result_attachments = function(results_model) {
      ...
      for (attachment in names(self$result_attachment_functions)) {
          ...
          suppressWarnings(
            withCallingHandlers({
              ...
              if (length(grep("Primitive", deparse(attachment_function), fixed = TRUE))) { # use default
                attachment_value <- attachment_function(results_model$default)
              } else { # apply function to results
                attachment_value <- attachment_function(results_model)
              }
            }, warning = function(w) {
              results_model$warning_messages <- c(results_model$warning_messages,
        ...
        error = function(e){
          results_model$error_messages <- c(results_model$error_messages,
                                            paste(sprintf("Error encountered attaching %s to results.", attachment), as.character(e)))
        })

        # Attach the returned function value to the results model
        if (!is.null(attachment_value)) {
          results_model$attached[[attachment]] <- attachment_value
        }
      }
    },
    ...
    #' @param results_model The sample results model, an object of a class inherited from \emph{GenericResultsModel}.
    ...
    calculate_summaries = function(results_model, sample_index) {
      ...
      for (name in c(self$summary_metrics, self$summary_matrices)) {
        ...
        if (!(name %in% names(self$summary_functions))) {
          results_model$error_messages <- c(results_model$error_messages, sprintf("No %s function defined for %s", type, name))
        } else {
          function_value <- self$summary_functions[[name]]
          data_value <- NULL
          if (is.numeric(function_value)) { # constant value
            data_value <- function_value
          } else if (is.character(function_value) && function_value %in% results_model$get_attribute_names(all = TRUE)) { # results attribute
            data_value <- eval(parse(text = paste0("results_model$", function_value)))
              ...
              suppressWarnings(
                withCallingHandlers({
                  ...
                  if (length(grep("Primitive", deparse(function_value), fixed = TRUE))) { # use default
                    data_value <- function_value(results_model$default)
                  } else { # apply function to results
                    data_value <- function_value(results_model)
                  }
                }, warning = function(w) {
                  results_model$warning_messages <- c(results_model$warning_messages,
            ...
            error = function(e){
              results_model$error_messages <- c(results_model$error_messages,
                                                paste(sprintf("Error encountered setting %s %s.", type, name), as.character(e)))
            })
          }
          if (!is.null(data_value)) {
            if (type == "metric") {
              if (length(data_value) == 1) {
                ...
                results_model$error_messages <- c(results_model$error_messages, paste("Metric function defined for", name, "produces multiple values"))
              }
            } else { # matrix
      ...
      generation <- list(successful = (length(results_model$error_messages) == 0))
      error_warning <- paste0(c("errors", "warnings")[which(c((length(results_model$error_messages) > 0), (length(results_model$warning_messages) > 0)))], collapse = " and ")
      if (length(results_model$error_messages) || length(results_model$warning_messages)) {
      ...
      if (length(results_model$error_messages)) {
        generation$errors <- results_model$error_messages
      }
      if (length(results_model$warning_messages)) {
        generation$warnings <- results_model$warning_messages
      }
    ...
    #' @description
    #' Summarizes the log generated within the generate method and writes it to (text and RDS) files in the results directory.
      ...
      suppressWarnings(try({
        ...
        if (length(generation_log$failed_indices)) {
          ...
          for (i in generation_log$failed_indices) {
            writeLines(c("", generation_log$full_log[[i]]$message), con = file_con)
        ...
        saveRDS(generation_log, file = file.path(self$results_dir, "generation_log.RDS"))
      }, silent = TRUE))
      return(generation_log)
    },
  ...
  private = list(
    ...
    # Manager attributes #
    .manager_attributes = c("sample_data", "results_model", "niche_k_model", "human_density_model",
                            "result_attachment_functions", "summary_metrics", "summary_matrices", "summary_functions",
                            "summary_metric_data", "summary_matrix_list", "summary_matrix_weighted_averages",
  ...
  active = list(
    ...
    #' @field results_model An object of a class inherited from the GenericResultsModel class for encapsulating and dynamically generating simulation results.
    results_model = function(value) {
      if (missing(value)) {
        private$.results_model
      } else {
        if (!is.null(value) && !("GenericResultsModel" %in% class(value))) {
          stop("The results model must be a object of a class inherited from the GenericResultsModel class", call. = FALSE)
        } else {
          private$.results_model <- value
        }
      }
    },
    ...
    #' @field result_attachment_functions A list of functions for attaching intermediate values to the results model prior to generation.
        ...
        for (attachment in names(value)) {
          ...
          if (is.character(attachment_value) && file.exists(attachment_value) && length(grep(".R", toupper(attachment_value), fixed = TRUE))) {
            ...
            if (is.function(attachment_function)) {
              ...
              stop(paste("Could not assign function from", attachment_value), call. = FALSE)
            }
    ...
    summary_functions = function(value) {
      if (missing(value)) {
        ...
        for (metric in names(value)) {
          ...
          if (is.character(metric_value) && !is.null(self$results_model) && metric_value %in% self$results_model$get_attribute_names(all = TRUE)) {
            metric_functions[[metric]] <- metric_value # direct results model attribute
            ...
            if (is.function(metric_function)) {
              ...
              stop(paste("Could not assign function from", metric_value), call. = FALSE)
            }
          } else { # numeric, character or direct assignment
