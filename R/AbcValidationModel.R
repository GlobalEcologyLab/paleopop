#' R6 Class Representing a Approximate Bayesian Computation (ABC) Model.
#'
#' @description
#' An R6 class to to represent a model for Approximate Bayesian Computation (ABC) 
#' validation and best simulation model selection. The class wraps functionality from
#' the \emph{abc} library, and includes methods for resolving non-finite metrics, 
#' centering and scaling the ABC inputs, running the ABC analysis, and generating ABC
#' diagnostics.
#'
...
AbcValidationModel <- R6Class("AbcValidationModel",
  ...
  public = list(
    ...
    save_to_rds = function(path = NULL) {
      ...
      } else if (!is.null(self$output_dir)) {
        super$save_to_rds(self$output_dir)
      } else {
        stop(paste("To save the model the file path or output directory needs to be provided"), call. = FALSE)
      }
    },
    ...
    #' @description
    #' Pre-processes inputs, runs ABC validation for input parameters, and stores abc (and optionally diagnostic) outputs (see \emph{abc} documentation).
    #' @param param A data frame of sample model parameters for each simulation.
    #' @param sumstat A data frame of result summary metrics for each simulation.
    #' @param target A vector of observed targets for each summary metric.
    #' @param tol Tolerance or proportion of models to select.
    #' @param method ABC algorithm to be applied (default is a neural network algorithm - see \emph{abc} documentation) .
    #' @param output_diagnostics Boolean to indicate whether or not to output diagnostics (PDF file - default is FALSE).
    #' @param ... Additional ABC parameters passed individually (see \emph{abc} documentation).
    run_abc = function(param = NULL, sumstat = NULL, target = NULL, tol = 0.01, method = "neuralnet", output_diagnostics = FALSE, ...) {

      # Ensure sample model parameters, result summary metrics and observed targets are present
      if (!is.null(param)) {
        self$simulation_parameters <- param
      }
      if (!is.null(sumstat)) {
        self$simulation_summary_metrics <- sumstat
      }
      if (!is.null(target)) {
        self$observed_metric_targets <- target
      }
      ...
      if (length(missing_parameters)) {
        stop(paste("ABC validation run requires parameters to be set first:", paste(missing_parameters, collapse = ", ")), call. = FALSE)
      }
      ...
      self$center_scale_abc_inputs()

      # Randomize the simulation sample indices for the ABC inputs
      if (!is.null(self$random_seed)) {
        set.seed(self$random_seed)
      }
      self$random_indices <- sample(x = seq(1, nrow(self$simulation_parameters), 1), size = nrow(self$simulation_parameters), replace = FALSE)

      # Run the ABC function
      run_status <- tryCatch({
        suppressWarnings(
          withCallingHandlers({
            self$abc_return_object <- abc::abc(
              target = self$observed_metric_targets,
              param = self$simulation_parameters[self$random_indices,],
              sumstat = self$simulation_summary_metrics[self$random_indices,],
              tol = tol, method = method, ...)
          }, warning = function(w) {
            self$warning_messages <- c(self$warning_messages,
                                       gsub("simpleWarning", "ABC function generated warning",
      ...
      error = function(e){
        list(stop = sprintf("ABC validation function failed. %s", as.character(e)))
      })
      ...
      if (output_diagnostics) {
        if (!is.null(self$output_dir)) {
          if (method %in% c("loclinear", "neuralnet")) {
            self$generate_abc_diagnostics()
          } else {
            warning("ABC diagnostics can only be generated with ABC method neuralnet or loclinear", call. = FALSE)
          }
        } else {
          warning("The output directory needs to be set before ABC diagnostics can be generated", call. = FALSE)
        }
      }
    ...
    center_scale_abc_inputs = function() {
    ...
    #' @description
    #' Generates the ABC diagnostics (see \emph{abc} documentation) as a PDF file in the output directory.
    #' @param output_dir Output directory path for the ABC diagnostics PDF file.
    generate_abc_diagnostics = function(output_dir = NULL) {

      # Ensure the ABC  return object and output directory are set
      if (!is.null(output_dir)) {
        self$output_dir <- output_dir
      }
      missing_messages <- c("ABC validation to be run", "and the", "output directory to be set")
      missing_messages <- paste(missing_messages[which(c(is.null(self$abc_return_object),
                                    is.null(self$abc_return_object) && is.null(self$output_dir),
                                    is.null(self$output_dir)))], collapse = " ")
      if (missing_messages != "") {
        stop(sprintf("ABC diagnostics generation requires the %s first", missing_messages), call. = FALSE)
      }

      # Diagnostics plot can only be generated when method is "loclinear" or "neuralnet"
      if (!(self$abc_return_object$method %in% c("loclinear", "neuralnet"))) {
        stop("ABC diagnostics can only be generated when ABC methods neuralnet or loclinear were utilized", call. = FALSE)
      }
      ...
      diag_status <- tryCatch({
        suppressWarnings(
          withCallingHandlers({
            pdf(file = file.path(self$output_dir, "abc_diagnostics.pdf"), onefile = TRUE)
            plot(self$abc_return_object, self$simulation_parameters, subsample = nrow(self$simulation_parameters), ask = FALSE)
            pdf_device_off <- dev.off()
          }, warning = function(w) {
            self$warning_messages <- c(self$warning_messages,
                                       gsub("simpleWarning", "ABC diagnostics generation warning",
                                            gsub("\n", "", as.character(w), fixed = TRUE),
                                            fixed = TRUE))
          })
        )
      ...
      error = function(e){
        list(stop = sprintf("ABC diagnostics generation failed. %s", as.character(e)))
      })
      if (!is.null(self$warning_messages)) {
        warning(self$warning_messages, call. = FALSE)
        self$warning_messages <- NULL
  ...
  private = list(
    ...
    .model_attributes = c("simulation_parameters", "simulation_summary_metrics", "observed_metric_targets",
                          ...
                          "input_center_scale_values", "output_dir", "abc_return_object",
  ...
  active = list(
    ...
    #' @field random_seed A seed for randomizing the order of the simulation samples.
    ...
    #' @field random_indices Randomized simulation sample indices for the ABC inputs and consequently the ABC results.
    ...
    #' @field input_center_scale_values A nested list of center and scale values for ABC input parameters/metrics.
    ...
    #' @field output_dir Directory path for ABC regression diagnostic and other outputs.
      ...
      if (missing(value)) {
        ...
        if (!is.null(value)) {
          ...
          if (!dir.exists(value)) { # create directory
            suppressWarnings(try(dir_created <- dir.create(value, recursive = TRUE), silent = TRUE))
            if (!dir_created) {
              stop(paste("Could not create output directory", value), call. = FALSE)
            }
          }
        }
    ...
    #' @field abc_return_object Object returned by the ABC function (see abc documentation).
    abc_return_object = function(value) {
      if (missing(value)) {
        private$.abc_return_object
      } else {
        private$.abc_return_object <- value
      }
    },
    
    #' @field selected_simulations A data frame of simulation sample indices and weights selected/assigned by the ABC validation.
    selected_simulations = function(value) {
      if (missing(value)) {
        if (!is.null(self$random_indices) && !is.null(self$abc_return_object) && 
            !is.null(self$abc_return_object$weights) && !is.null(self$abc_return_object$region)) {
          selected_simulations <- data.frame(index = self$random_indices[which(self$abc_return_object$region)],
                                         weight = self$abc_return_object$weights)
