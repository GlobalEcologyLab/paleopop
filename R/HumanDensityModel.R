...
HumanDensityModel <- R6Class("HumanDensityModel",
  ...
  public = list(
    ...
    #' @description
    #' Calculates the human density values via the (triangular) sampling distribution for each simulation grid and time step for each grid cell.
    ...
    calculate_human_density = function() {
      ...
      # Ensure single number sample is a number between 0 and 1
      if (!is.null(self$human_density_sample) && (!is.numeric(self$human_density_sample) || self$human_density_sample < 0 || self$human_density_sample > 1)) {
      ...
      if (!is.null(self$distrib_data) && !is.null(self$human_density_sample)) {
        ...
        human_densities[distrib_data$index] <- metRology::qtri(self$human_density_sample,
  ...
  private = list(
    ...
    .model_attributes = c("carrying_capacity_mean", "carrying_capacity_sd", "human_occupancy_mask",
                          ...
                          "decimals", "human_density_sample", "human_densities"),
      ...
      if (missing(value)) {
        if (is.null(private$.human_densities) && "human_densities" %in% names(self$generative_requirements)) {
          ...
          if (template_type == "file") {
            ...
            message <- self$calculate_human_density()
