...
This part of the workflow is identical to `poems`: we create `Generator` objects to dynamically generate dispersal between grid cells and habitat suitability at each point in space and time.
...
as well as, optionally, a conductance/friction landscape that maps out barriers
to dispersal, such as ice or mountains. (We will omit the friction landscape for this example.)
...
suitable). `paleopop` has a spatiotemporally explicit habitat suitability `RasterStack` for Siberia from
21,000 - 9,000 years BP.
...
Normally we would have spatiotemporally dynamic data on human density over time, which the prey- and predator-dependent harvest function translates into numbers of bison harvested, but in the interest of simplicity for this example, we will create a matrix of constant low human density.
...
Here the workflow is the same as in `poems`: we sample evenly along a range of values so as to create an uninformative prior, and use these to parameterize different simulations. For the sake of this example, we will sample 100 different parameter combinations, but it is advisable to run more in order to fully sample the parameter space.
...
The simulation log, `run_output`, can be examined for error messages and failure indices if any of the simulations fail.

# 6. Examine the results

We can explore the results of the simulations using the convenient wrapper of the `PaleoPopResults` object. Although we selected our result output as "abundance" in the model template above, we can look at other result metrics, some of which are unique to `PaleoPopResults`, via the `ResultsManager`.
...
Looking at these results, abundance trend could be a suitable metric to converge toward a validation target, since there is a spread of outcomes, but extinction time would not be ideal for validation, since the outcomes are binary: either the population rapidly becomes extinct, or it persists for the whole simulation.

From here, you may continue with the `poems` workflow, creating a `Validator` object for pattern-oriented modeling to validate the results of the simulation.